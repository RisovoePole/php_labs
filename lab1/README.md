...
1. GET-запрос к серверу по адресу http://sandbox.usm.com, в заголовке User-Agent "Ainisimov Victor" \
`GET http://sandbox.usm.com User-Agent:"Ainisimov Victor"` \
...
3. Составьте PUT-запрос к серверу по адресу http://sandbox.usm.com/cars/1, указав в заголовке User-Agent ваше имя и фамилию, в заголовке Content-Type значение application/json и в теле запроса следующие параметры:
    ...
    ``` http
    PUT http://sandbox.usm.com/cars/1 User-Agent:"Victor Anisimov" Content-Type:application/json 
    make = Toyota
    model = Corolla
    year := 2021
    ``` 
